...
import { BookOpen, User, Calendar, CreditCard, Sparkles, ShieldCheck, X, Star, Video, Play, FileText } from 'lucide-react';
    ...
    >
      ...
      <div className="p-6 flex-1 flex flex-col justify-between">
        ...
        <div className="space-y-3">
          ...
          {(item.meetLink || item.videoUrl || item.materialsUrl) && (
            <div className="pt-3 border-t border-slate-100 flex flex-wrap gap-2 text-[11px]">
              {item.meetLink && (
                <a 
                  ...
                  className="px-2.5 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg font-bold hover:bg-emerald-100 transition-colors flex items-center gap-1"
                >
                  <Video className="w-3 h-3" /> Live Meet
                </a>
              )}
              {item.videoUrl && (
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="px-2.5 py-1 bg-purple-50 text-purple-700 border border-purple-200 rounded-lg font-bold hover:bg-purple-100 transition-colors flex items-center gap-1"
                >
              ...
              {item.materialsUrl && (
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="px-2.5 py-1 bg-blue-50 text-blue-700 border border-blue-200 rounded-lg font-bold hover:bg-blue-100 transition-colors flex items-center gap-1"
                >
        ...
        <div className="flex gap-2 mt-5">
          <button
            onClick={() => {
              syncClassToGoogleCalendar(item);
              showToast(`Google Calendar invite opened for "${item.title}"`, "info");
            }}
            className="p-2.5 rounded-xl border border-indigo-200 text-indigo-700 bg-indigo-50 hover:bg-indigo-100 transition-all cursor-pointer flex items-center justify-center"
