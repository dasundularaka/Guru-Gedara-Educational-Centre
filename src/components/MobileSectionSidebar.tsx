...
export const MobileSectionSidebar: React.FC<MobileSectionSidebarProps> = ({
  ...
  return (
    <>
      {/* Mobile Top Section Selector Trigger Bar */}
      <div className="block md:hidden mb-4">
        <button
          type="button"
          onClick={onOpen}
          className="w-full flex items-center justify-between p-3.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm hover:shadow-md transition-all active:scale-[0.99] touch-target cursor-pointer group"
          id="mobile_section_sidebar_trigger"
        >
          <div className="flex items-center gap-3 min-w-0">
            <div className="p-2 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 rounded-xl shrink-0 group-hover:scale-105 transition-transform">
              {activeItem?.icon || <Layers className="w-5 h-5" />}
            </div>
            <div className="text-left min-w-0">
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] uppercase font-mono font-extrabold text-slate-400 dark:text-slate-500 tracking-wider">
                  Active Section
                </span>
                <span className={`w-1.5 h-1.5 rounded-full ${roleBadgeColor}`} />
              </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/50 px-2.5 py-1 rounded-xl">
              Switch
            </span>
      ...
      {/* Modern Slide-In Sidebar Drawer */}
      <AnimatePresence>
        {isOpen && (
