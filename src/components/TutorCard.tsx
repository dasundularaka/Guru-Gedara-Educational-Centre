...
export const TutorCard: React.FC<TutorCardProps> = ({ tutor }) => {
  ...
  return (
    <motion.div
      ...
      className="sleek-card p-4 sm:p-6 flex flex-col justify-between h-full bg-white"
      id={`tutor_card_${tutor.uid}`}
    >
      <div>
        {/* Profile Header */}
        <div className="flex gap-3 sm:gap-4 items-start mb-3 sm:mb-5 relative">
          {tutor.photoURL ? (
            <img 
              referrerPolicy="no-referrer"
              className="h-12 w-12 sm:h-14 sm:w-14 rounded-2xl object-cover ring-2 ring-slate-100 shrink-0" 
              src={tutor.photoURL} 
              alt={tutor.name} 
            />
          ) : (
            <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-2xl bg-slate-100 text-slate-800 font-extrabold flex items-center justify-center text-base sm:text-lg shrink-0">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1 flex items-center">
              <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[8px] sm:text-[9px] font-black tracking-wide uppercase border ${
            ...
            <div className="flex items-center gap-1.5 mt-1.5">
              <button 
                ...
                className="carousel-dot btn-compact flex items-center gap-1 bg-amber-50 hover:bg-amber-100 border border-amber-100 px-1.5 sm:px-2 py-0.5 rounded-lg text-[11px] sm:text-xs font-bold text-slate-800 transition-colors cursor-pointer"
        ...
        {/* Bio */}
        <p className="text-[11px] sm:text-xs text-slate-500 leading-relaxed line-clamp-2 sm:line-clamp-3 mb-3 sm:mb-5">
          {details.bio}
        </p>
        ...
        <div className="space-y-2 mb-3 sm:mb-5 bg-slate-50 p-2.5 sm:p-3.5 rounded-xl sm:rounded-2xl border border-slate-100 text-[10px] sm:text-[11px] text-slate-600">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="truncate font-bold text-emerald-700 uppercase tracking-wide text-[8px] sm:text-[9px] font-mono">Verified Faculty Member</span>
          </div>
        </div>
